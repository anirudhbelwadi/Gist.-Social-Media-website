...
def signup():
    ...
    if(request.method == 'POST'):
        ...
        if(password != confpass):
            ...
            details = cur.fetchall()
            if(len(details) == 0):
                con.execute("INSERT INTO Users (username, email, password, id) VALUES (?, ?, ?, ?)",
...
def events():
    if(request.method == 'POST'):
        ...
        con = sqlite3.connect('database.db')
        con.execute("INSERT INTO events (username, eventName, date, startTime, endTime, description, eventId) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (username, eventname, date, stime, etime, descrip, eventId))
        con.commit()
        con.close()
...
def sendfrequest():
    if request.method == 'POST':
        ...
        con = sqlite3.connect('database.db')
        con.execute(
            "INSERT INTO frequest (fromUser, toUser) VALUES (?, ?)", (username, fusername))
        con.commit()
        con.close()
...
def accept():
    if request.method == 'POST':
        ...
        con = sqlite3.connect('database.db')
        con.execute(
            "UPDATE Users SET friends = ? WHERE username = ?", (new_u, username))
        con.execute(
            "UPDATE Users SET friends = ? WHERE username = ?", (new_f, fusername))
        con.execute(
            "DELETE FROM frequest WHERE fromUser = ? AND toUser = ?", (fusername, username))
        con.commit()
        con.close()
...
def tweets():
    if request.method == 'POST':
        ...
        if request.files['fileupload']:
            ...
            f.save(uploadedIMG)
            con = sqlite3.connect('database.db')
            con.execute("INSERT INTO tweets (username, message, postTime, Id, filename) VALUES (?, ?, ?, ?, ?)",
                        (username, message, posttime, Id, f_name))
            con.commit()
            con.close()
        else:
            con = sqlite3.connect('database.db')
            con.execute("INSERT INTO tweets (username, message, postTime, Id) VALUES (?, ?, ?, ?)",
                        (username, message, posttime, Id))
            con.commit()
            con.close()
    if 'id' in session:
        username = session['username']
...
@app.route('/eventInvitations', methods = ['GET'])
def eventInvitations():
    if request.method == 'GET':
        con = sqlite3.connect('database.db')
        cur = con.cursor()
        cur.execute("SELECT * FROM eventInvitation WHERE username = ?", (session['username'],))
        data = cur.fetchall()
        print(data)
        con.close()
        return render_template('eventInvitations.html', data=data)


@app.route('/declineInvitation', methods = ['POST'])
def declineInvitation():
    if request.method == 'POST':
        ...
        con = sqlite3.connect('database.db')
        cur = con.cursor()
        cur.execute("DELETE FROM eventInvitation WHERE username = ? AND eventId = ?", (username, eventId))
        con.commit()
        con.close()
        return redirect(url_for('eventInvitations'))
...
def statusupdate():
    if request.method == "POST":
        ...
        con = sqlite3.connect('database.db')
        con.execute("UPDATE Users SET status = ? WHERE username = ?",
                    (status, username))
        con.commit()
        con.close()
...
def done():
    if request.method == 'POST':
        ...
        con = sqlite3.connect('database.db')
        con.execute("UPDATE events SET done = ? WHERE eventId = ?",
                    (done, eventId))
        con.commit()
        con.close()
        return redirect(url_for('events'))
